...
function Export-DashboardJsonCompat {
    ...
    $warningCount = @($Rows | Where-Object { $ErrorActionPreference = "Stop"
...
function Export-DashboardJsonCompat {
    ...
    if ($Rows.Count -gt 0) {`r`n        $finalStatus = "PRODOTTO_STRUTTURATO_MULTIROW"`r`n    }`r`n    else {`r`n        $finalStatus = "ATTENZIONE"`r`n    }
...
}.stato_generale -eq "ATTENZIONE" -or $ErrorActionPreference = "Stop"
...
try {
    ...
    Write-Host ""
    Write-Host "[WARNING] Export dashboard JSON fallito: $($_.Exception.Message)"
}.stato_generale -eq "DA_COMPLETARE" }).Count
    $errorCount = @($Rows | Where-Object { $ErrorActionPreference = "Stop"
...
try {
    ...
    Write-Host ""
    Write-Host "[WARNING] Export dashboard JSON fallito: $($_.Exception.Message)"
}.stato_generale -eq "CRITICO" }).Count
    $okCount = @($Rows | Where-Object { $ErrorActionPreference = "Stop"
...
try {
    ...
    $summary = "Contratto REGINA. Record dashboard: $($Rows.Count). OK: $okCount. Warning: $warningCount. Errori: $errorCount."
