...
function Get-CategoriaArchivistica {
    ...
    if ($name -match 'datenblatt|datasheet|scheda tecnica|documentazione tecnica|registro dpi|scadenziario|inventario|matricola|operatore|cliente|dpi') {
        return [pscustomobject]@{ Categoria = "DOCUMENTI_CLIENTE"; Fonte = "AUTOMATICA"; Confidence = 80 }
    }

    return [pscustomobject]@{ Categoria = "DA_VALIDARE"; Fonte = "AUTOMATICA"; Confidence = 40 }
}
...
function Get-EvidenzeOperative {
    ...
    if ($signals.Count -ge 2) { $containsOperationalData = $true }
    if (($signals -contains "SERIALE") -and ($signals -contains "VERIFICA")) { $containsOperationalData = $true }
    if (($signals -contains "OPERATORE") -and ($signals -contains "VERIFICA")) { $containsOperationalData = $true }
...
function Get-PrioritaDashboard {
    ...
    if ($ContieneDatiOperativi) {
        return [pscustomobject]@{ Priorita = "DASHBOARD_IMMEDIATA"; Confidence = 95 }
    }

    if ($CategoriaArchivistica -eq "REVISIONI") {
        return [pscustomobject]@{ Priorita = "DASHBOARD_IMMEDIATA"; Confidence = 90 }
    }

    if ($CategoriaArchivistica -eq "DOCUMENTI_CLIENTE" -or $CategoriaArchivistica -eq "CERTIFICATI") {
        return [pscustomobject]@{ Priorita = "DASHBOARD_STANDARD"; Confidence = 75 }
    }

    return [pscustomobject]@{ Priorita = "SOLO_ARCHIVIO"; Confidence = 60 }
}
...
function Export-DashboardJsonCompat {
    ...
    if ($Rows.Count -gt 0) {
        $finalStatus = "PRODOTTO_STRUTTURATO_MULTIROW"
    }
    else {
...
if (Test-Path $operatoriDir) {
    ...
    foreach ($op in $ops) {
        ...
        if (Test-Path $csv) { Write-Host "OK SCHEDA OPERATORE ->" $op.Name }
        else { Write-Host "MANCANTE SCHEDA ->" $op.Name }
    }
}
else {
...
foreach ($file in $files) {
    ...
    if ($prio.Priorita -eq "DASHBOARD_IMMEDIATA") { $status = "OK" }
    elseif ($prio.Priorita -eq "DASHBOARD_STANDARD") { $status = "ATTENZIONE" }
...
if (Test-Path $reportFile) {
    ...
    $dashboardCheck = "CONTRATTO REGINA ATTESO"
